...
void eval(char *cmdline) 
{
	...
	bg = parseline(cmdline, argv); // & 가 있는지 확인하고 리턴
	...
	sigprocmask(SIG_BLOCK, &mask, &prev); // 시그널 블록
	...
	if(!builtin_cmd(argv)){
		...
		sigprocmask(SIG_SETMASK, &prev, NULL); // 시그널 언블록

		/* addjob을 하기 전에 자식 프로세스가 종료되면
		 * 부모는 제거된 job을 추가하는 꼴이된다.
		 * 따라서 fork 하기 전에 블록을 해놓고
		 * addjob 이후에 자식프로세스의 시그널을 받도록 설정하는 것이다*/

		if(!bg){ // foreground

			// waitpid 와 deletejob은 시그널 핸들러에서 구현
			while(1){
				if(pid != fgpid(jobs)){ 
					break;
				} else {
